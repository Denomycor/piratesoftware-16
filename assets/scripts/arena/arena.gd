...
var wall = preload("res://assets/scenes/levels/walls/Wall.tscn")
var corner = preload("res://assets/scenes/levels/walls/Corner.tscn")
...
var angle_increment = 2 * PI / NUMBER_OF_SIDES
...
func build_arena():
	...
	var radius = walls_per_side * WALL_LENGTH + CORNER_LENGTH * (NUMBER_OF_SIDES - 2)

	var center = position

	var last_position = to_global(center) + Vector2(radius, 0)

	var direction = Vector2(cos(angle_increment), sin(angle_increment)).rotated(PI / 2).normalized()
	...
	for i in range(NUMBER_OF_SIDES):
		...
		var corner_instance = corner.instantiate()
...
func build_wall(start_position: Vector2, direction: Vector2):
	var facing_angle = direction.rotated(PI / 2).angle()
