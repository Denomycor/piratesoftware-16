...
## Emitted whenever the car's HP changes (also on init). Wired by Level to Overlay.
signal health_changed(hp: float)
## Emitted when the car is destroyed. Wired by Level to set_game_over().
...
func apply_drift_friction() -> void:
...
func on_weapon_activated() -> void:
	pass

func on_weapon_deactivated() -> void:
	pass
