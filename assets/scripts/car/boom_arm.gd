...
## Percentage of shaking reduction per second
@export_range(0, 1) var shake_decay: float
...
func set_boom_position(delta: float) -> void:
	...
	var pos: Vector2 = (mouse_cam_distance * get_mouse_direction() + speed_cam_distance * owner.linear_velocity.normalized().rotated(-owner.rotation)) * slide_ratio
	position = lerp(position, pos, delta * cam_speed)

func set_camera_zoom(delta: float) -> void:
	var spd: float = owner.linear_velocity.length()
	var zoom := clampf(lerpf(max_zoom, min_zoom, (spd - min_speed) / max_speed), min_zoom, max_zoom)
